...
from testPhosphenes import beginEvaluating
...
parser.add_argument('--demo_type', type=str) # 'train', 'test'
...
if __name__ == 'main':
    ...
    sets = [set_1, set_2, set_3, set_4, set_5, set_6, set_7, set_8, set_9, set_10, set_11, set_12, set_13, set_14] # 14 independent sets where each of them contains different demographic distribution
    seed_list = [22, 77, 2, 100, 81, 42, 7, 1, 55, 50] # different 1,000 training images (total of 10 permutations)

    for m in range(14): # for each set
        ...
        for seed in seed_list:
            for n in range(len(com_list)): # for every combination of question set
                ...
                checkpoint_file = os.path.join(model_path, f'Checkpoint_{args.model_type2}{args.finetune}.pt') # for PCA_LR, CNN_LR, CNN_SVC
                earlystop_file = os.path.join(model_path, f'Early_Stop_{args.model_type2}{args.finetune}.pt') # for PCA_LR, CNN_LR, CNN_SVC
                ...
                if os.path.isfile(check_file) and args.xai == 'no':
                    ...
                    Xtrain, ytrain, _, old_uniq_labels, tr_unique_items = loadData(com_list[n], 'train', args.train_path, test_path, args.ft_path, args.finetune, args.data_type, accessory_list, light_list, expression_list, camera_list, seed, args.r, args.model_type2)
                    Xtest, ytest, file_path_list, old_uniq_labels2, test_unique_items = loadData(com_list[n], 'test', args.train_path, test_path, args.ft_path, args.finetune, args.data_type, accessory_list, light_list, expression_list, camera_list, seed, args.r, args.model_type2)
